...
void ADAPT::cleanup()
{
    ...
    vkDestroyImageView(device, resultImageView, nullptr);

    vkDestroyImage(device, resultImage, nullptr);
    vkFreeMemory(device, resultImageMemory, nullptr);
...
void ADAPT::createResultImage()
{
    ...
    createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, resultImageBuffer, resultImageBufferMemory);
    ...
    if (vkAllocateMemory(device, &allocInfo, nullptr, &resultImageMemory) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate image memory!");
    }

    vkBindImageMemory(device, resultImage, resultImageMemory, 0);

    transitionImageLayout(resultImage, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

    copyBufferToImage(stagingBuffer, resultImage, WIDTH, HEIGHT);

    transitionImageLayout(resultImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL);
...
void ADAPT::createResultSampler()
{
    ...
    {
        throw std::runtime_error("failed to create storage sampler!");
    }
}
...
void ADAPT::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
    ...
    const uint32_t localX = 16, localY = 16;
